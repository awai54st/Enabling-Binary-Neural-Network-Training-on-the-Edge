...
#include "../naive_layers_ovw/vanilla_layers/convolution/vanilla_convolution_ovw.h"
...
int main(int argc, char * argv[]) {
    ...
    if (EPOCH != 0) {
        std::vector<std::unique_ptr<BaseLayer<Matrix, float, Matrix<float>&>>> layer_seq(17);
        layer_seq[0] = std::make_unique<Vanilla_Convolution2D_OWH1D>(32, 3, "same", true, 0);
        layer_seq[1] = std::make_unique<Vanilla_BatchNormConv<Matrix, Matrix<float>&>>(BATCH_NORM_MOMENTUM);
        layer_seq[2] = std::make_unique<BinaryActivation<float, Matrix, float, Matrix<float>&>>();

        layer_seq[3] = std::make_unique<Vanilla_Convolution2D_OWH1D>(32, 3, "same", false, 1);
        layer_seq[4] = std::make_unique<Vanilla_BatchNormConv<Matrix, Matrix<float>&>>(BATCH_NORM_MOMENTUM);
        layer_seq[5] = std::make_unique<BinaryActivation<float, Matrix, float, Matrix<float>&>>();
        layer_seq[6] = std::make_unique<MaxPooling<float, Matrix, float, Matrix<float>&>>(2, 2);

        layer_seq[7] = std::make_unique<Flatten<Matrix, float, Matrix<float>&>>();

        layer_seq[8] = std::make_unique<Vanilla_Dense_OVW1D>(64, false, 0);
        layer_seq[9] = std::make_unique<Vanilla_BatchNormDense<Matrix, Matrix<float>&>>(BATCH_NORM_MOMENTUM);
        layer_seq[10] = std::make_unique<BinaryActivation<float, Matrix, float, Matrix<float>&>>();

        layer_seq[11] = std::make_unique<Vanilla_Dense_OVW1D>(64, false, 1);
        layer_seq[12] = std::make_unique<Vanilla_BatchNormDense<Matrix, Matrix<float>&>>(BATCH_NORM_MOMENTUM);
        layer_seq[13] = std::make_unique<BinaryActivation<float, Matrix, float, Matrix<float>&>>();

        layer_seq[14] = std::make_unique<Vanilla_Dense_OVW1D>(10, false, 3);
        layer_seq[15] = std::make_unique<Vanilla_BatchNormDense<Matrix, Matrix<float>&>>(BATCH_NORM_MOMENTUM);
        layer_seq[16] = std::make_unique<Softmax<float, Matrix, Matrix<float>&>>();

        CrossEntropy<float, Matrix> cross_entropy_1;
        Adam<float, Matrix<float>&> Adam_opt(1e-3, 0.9, 0.999, 10);
        ...
        for (int i=0; i<EPOCH; i++) {
            {
                ...
                Matrix<float> mnist_in({BATCH_SIZE, 784});
                read_csv<float>(input_filename, mnist_in, {BATCH_SIZE,784}, 0);
