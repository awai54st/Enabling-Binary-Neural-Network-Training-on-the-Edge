...
int main(int argc, char * argv[]) {
    ...
    layer_seq.push_back(std::make_unique<OPT_Vanilla_Convolution2D>(128, 3, "same", true, 0));
    layer_seq.push_back(std::make_unique<Vanilla_BatchNormConv<Matrix>>(BATCH_NORM_MOMENTUM));
    layer_seq.push_back(std::make_unique<BinaryActivation<float, Matrix>>());
    layer_seq.push_back(std::make_unique<OPT_Vanilla_Convolution2D>(128, 3, "same", false, 0));
    layer_seq.push_back(std::make_unique<MaxPooling<float, Matrix>>(2, 2));
    layer_seq.push_back(std::make_unique<Vanilla_BatchNormConv<Matrix>>(BATCH_NORM_MOMENTUM));
    layer_seq.push_back(std::make_unique<BinaryActivation<float, Matrix>>());
    
    layer_seq.push_back(std::make_unique<OPT_Vanilla_Convolution2D>(256, 3, "same", false, 0));
    layer_seq.push_back(std::make_unique<Vanilla_BatchNormConv<Matrix>>(BATCH_NORM_MOMENTUM));
    layer_seq.push_back(std::make_unique<BinaryActivation<float, Matrix>>());
    layer_seq.push_back(std::make_unique<OPT_Vanilla_Convolution2D>(256, 3, "same", false, 0));
    layer_seq.push_back(std::make_unique<MaxPooling<float, Matrix>>(2, 2));
    ...
    layer_seq.push_back(std::make_unique<OPT_Vanilla_Convolution2D>(512, 3, "same", false, 0));
    layer_seq.push_back(std::make_unique<Vanilla_BatchNormConv<Matrix>>(BATCH_NORM_MOMENTUM));
    layer_seq.push_back(std::make_unique<BinaryActivation<float, Matrix>>());
    layer_seq.push_back(std::make_unique<OPT_Vanilla_Convolution2D>(512, 3, "same", false, 0));
    layer_seq.push_back(std::make_unique<MaxPooling<float, Matrix>>(2, 2));
    ...
    layer_seq.push_back(std::make_unique<Flatten<Matrix>>());
    
    layer_seq.push_back(std::make_unique<OPT_Vanilla_Dense>(1024, false, 0));
    layer_seq.push_back(std::make_unique<Vanilla_BatchNormDense<Matrix>>(BATCH_NORM_MOMENTUM));
    layer_seq.push_back(std::make_unique<BinaryActivation<float, Matrix>>());
    
    layer_seq.push_back(std::make_unique<OPT_Vanilla_Dense>(1024, false, 1));
    layer_seq.push_back(std::make_unique<Vanilla_BatchNormDense<Matrix>>(BATCH_NORM_MOMENTUM));
    layer_seq.push_back(std::make_unique<BinaryActivation<float, Matrix>>());
    
    layer_seq.push_back(std::make_unique<OPT_Vanilla_Dense>(10, false, 2));
    layer_seq.push_back(std::make_unique<Vanilla_BatchNormDense<Matrix>>(BATCH_NORM_MOMENTUM));
    ...
    for (int i=0; i<EPOCH; i++) {
        {
            ...
            Matrix<float> mnist_in = read_csv<float>(input_filename, {BATCH_SIZE,32*32*3}, 0, BATCH_SIZE*32*32*3);
...

        {
            ...
            Matrix<float> mnist_label = read_csv<float>(label_filename, {BATCH_SIZE,10}, 0, BATCH_SIZE*10);
            ...
            Matrix<float> mnist_in = read_csv<float>(input_filename, {BATCH_SIZE,32*32*3}, 0);
            mnist_in.reshape(BATCH_SIZE, 32, 32, 3);
