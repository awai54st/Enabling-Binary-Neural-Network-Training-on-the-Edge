...
int main(int argc, char * argv[]) {
    ...
    std::vector<std::unique_ptr<BaseLayer<Matrix, float16_t>>> layer_seq(17);
    
    layer_seq[0] = std::make_unique<OPT_XNor_Convolution2D>(32, 3, "same", true, 0);
    layer_seq[1] = std::make_unique<XNor_BatchNormConv<Matrix>>(BATCH_NORM_MOMENTUM);
    layer_seq[2] = std::make_unique<BinaryActivation<bool, Matrix, float16_t>>();
    
    layer_seq[3] = std::make_unique<OPT_XNor_Convolution2D>(32, 3, "same", false, 1);
    layer_seq[4] = std::make_unique<XNor_BatchNormConv<Matrix>>(BATCH_NORM_MOMENTUM);
    layer_seq[5] = std::make_unique<BinaryActivation<bool, Matrix, float16_t>>();
    layer_seq[6] = std::make_unique<MaxPooling<bool, Matrix, float16_t>>(2, 2);

    layer_seq[7] = std::make_unique<Flatten<Matrix, float16_t>>();
    
    layer_seq[8] = std::make_unique<OPT_XNor_Dense>(64, false, 0);
    layer_seq[9] = std::make_unique<XNor_BatchNormDense<Matrix>>(BATCH_NORM_MOMENTUM);
    layer_seq[10] = std::make_unique<BinaryActivation<bool, Matrix, float16_t>>();
    
    layer_seq[11] = std::make_unique<OPT_XNor_Dense>(64, false, 1);
    layer_seq[12] = std::make_unique<XNor_BatchNormDense<Matrix>>(BATCH_NORM_MOMENTUM);
    layer_seq[13] = std::make_unique<BinaryActivation<bool, Matrix, float16_t>>();
    
    layer_seq[14] = std::make_unique<OPT_XNor_Dense>(10, false, 2);
    layer_seq[15] = std::make_unique<XNor_BatchNormDense<Matrix>>(BATCH_NORM_MOMENTUM);
    layer_seq[16] = std::make_unique<Softmax<float16_t, Matrix>>();
    ...
    for (int i=0; i<EPOCH; i++) {
        {
            ...
            Matrix<float16_t> mnist_in = read_csv<float16_t>(input_filename, {BATCH_SIZE,784}, 0, BATCH_SIZE*784*32);
...

        {
            ...
            Matrix<float16_t> mnist_label = read_csv<float16_t>(label_filename, {BATCH_SIZE,10}, 0, BATCH_SIZE*784);
