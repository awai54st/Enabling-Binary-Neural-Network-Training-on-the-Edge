...
OPT_XNor_Convolution2D::OPT_XNor_Convolution2D(size_t filters, size_t kernel_size, std::string padding, bool is_first_layer, int random_seed): filters(filters), kernel_size(kernel_size), padding(padding), is_first_layer(is_first_layer), random_seed(random_seed) {};

OPT_XNor_Convolution2D::~OPT_XNor_Convolution2D() {};


Matrix<float16_t> OPT_XNor_Convolution2D::forward(Matrix<float16_t> & x, bool is_training) {
    x_shape = x.shape();
    ...
    convolution<float16_t, bool>(x, packed_kernel, output_shape);
    
    return x;
}


Matrix<float16_t> OPT_XNor_Convolution2D::backprop(Matrix<float16_t> & dy) {
    ...
    convolution<bool, float16_t, bool>(packed_x, dy, dkernel);
    ...
    convolution<float16_t, bool>(dy, packed_kernel, x_shape);
    
    return dy;
}


void OPT_XNor_Convolution2D::backprop(Matrix<float16_t> & dy, Matrix<float16_t> & original_input) {
    //const int8_t dy_bias = get_po2_bias<float16_t>(dy);
    //scale_with_po2_bias<float16_t>(dy, dy_bias);
    //log_quantize<float16_t>(dy, dy_bias, 4);
    //unsigned long long int one = 1;
    ...
    convolution<float16_t, float16_t, bool>(original_input, dy, dkernel);
