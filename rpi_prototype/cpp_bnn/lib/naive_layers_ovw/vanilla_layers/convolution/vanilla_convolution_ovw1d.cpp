...
Vanilla_Convolution2D_OWH1D::Vanilla_Convolution2D_OWH1D(size_t filters, size_t kernel_size, std::string padding, bool is_first_layer, int random_seed): filters(filters), kernel_size(kernel_size), padding(padding), is_first_layer(is_first_layer), random_seed(random_seed) {};

Vanilla_Convolution2D_OWH1D::~Vanilla_Convolution2D_OWH1D() {};


Matrix<float> &Vanilla_Convolution2D_OWH1D::forward(Matrix<float> & x, bool is_training) {
    x_shape = x.shape();
    ...
    _convolution_ovw<float, bool>(x, packed_kernel, output_shape, {1,1}, {pad_height,pad_width}, 0);
    return x;
}

// not first layer
Matrix<float> &Vanilla_Convolution2D_OWH1D::backprop(Matrix<float> & dy) {
    // dw
    ...
    _convolution_ovw<float, bool>(dy, packed_kernel, x_shape, {1,1}, {pad_height,pad_width}, 0);
    
    return dy;
}

// first layer
void Vanilla_Convolution2D_OWH1D::backprop(Matrix<float> &dy, Matrix<float> & original_input) {
    // dw
