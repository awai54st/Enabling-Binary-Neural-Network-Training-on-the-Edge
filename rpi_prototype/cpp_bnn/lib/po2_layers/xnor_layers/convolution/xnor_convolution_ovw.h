#ifndef xnor_convolution_ovw_h
#define xnor_convolution_ovw_h
...
class XNor_Convolution2D_OWH1D : public BaseLayer<Matrix, float16_t, Matrix<float16_t>&> {
    public:
        bool is_built = false;
        ...
        Matrix<bool> dkernel;
        //Constructors
        XNor_Convolution2D_OWH1D(size_t filters, size_t kernel_size, std::string padding="valid", bool is_first_layer=false, int random_seed=0);
        //Deconstructors
        ~XNor_Convolution2D_OWH1D();
        ...
        void backprop(Matrix<PO2_5bits_t> &dy, Matrix<float16_t> & original_input);
