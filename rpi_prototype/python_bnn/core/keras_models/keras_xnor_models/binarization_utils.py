...
def log2(x, eps=1e-37):
    numerator = tf.math.log(x+eps)
    denominator = tf.math.log(tf.constant(2, dtype=numerator.dtype))
    return numerator / denominator

def LOG_quantize(
        value, width):
    sign = K.sign(value + 1e-37) # Adding a small bias to remove zeros
...
class l1_batch_norm_mod_conv(Layer):
    ...
    def quantise_gradient_op(self, x):
        ...
        def custom_grad(dy):
            ...
            dy_norm_x = dy * ivar
            #print(dy_norm_x)
            #print(K.reshape(1.0/N * K.sum(dy_norm_x, axis=[0,1,2]), [1,1,1,-1]))
...
class l1_batch_norm_mod_dense(Layer):
    ...
    def quantise_gradient_op(self, x):
        ...
        def custom_grad(dy):
            ...
            #term_2 = result # Vanilla BN
            term_2 = K.sign(result+1e-37)
            #term_3 = 1.0/N * K.sum(dy_norm_x * result, axis=0) # Vanilla BN
            term_3 = 1.0/N * K.sum(dy_norm_x * term_2 * K.reshape(1.0/N * K.sum(K.abs(result), axis=0), [1,-1]), axis=0)
    ...
    def xnor_wg_conv_op(self, x, w):
        ...
        def custom_grad(dy):
            ...
            if self.first_layer:
                ...
                x_trans = K.sign(x_trans + 1e-37) # Adding bias 1e-37 to sign function
                dw = K.conv2d(x_trans, kernel=dy_trans, padding="valid",strides=self.strides )
            dw = tf.transpose(dw, [1,2,0,3])

            # Attenuate dW by sqrt of fanin
            N = self.k*self.k*self.ch_in*self.nfilters
            dw = 1./np.sqrt(N) * K.sign(dw + 1e-37) # Adding bias 1e-37 to sign function
    ...
    def xnor_wg_dense_op(self, x, w):
        ...
        def custom_grad(dy):
            ...
            if self.first_layer == True: # First layer's input activations are not binarised
                ...
                dw = K.dot(K.transpose(K.sign(x + 1e-37)), dy) # Adding bias 1e-8 to sign function
            
            # Attenuate dW by sqrt of fanin
            N = self.n_in*self.n_out
            dw = 1./np.sqrt(N) * K.sign(dw + 1e-37) # Adding bias 1e-37 to avoid zeros
